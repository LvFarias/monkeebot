...
// --- Configuration ---
const SPREADSHEET_ID = '1-sb9DqoSxpKMSFWBiKjVbF_Qv2VjAdVo-G8hDxUoaWQ';
...
export async function findPlayerForRerun(tabName, contractId, day, hours, isUltraOnly) {
	try {
		const sheetsClient = await getSheetsClient();
		...
		for (const rowIndexStr in ROW_TO_EST) {
			...
			if (
				userRows[rowIndex] &&
				hours.includes(ROW_TO_EST[rowIndex]) &&
				String(userRows[rowIndex][day + 1]).toUpperCase() === 'TRUE'
			) {
				user.hours.push(convertEstToUtc([ROW_TO_EST[rowIndex]]));
			}
		}
		return user;
	} catch (error) {
...
export async function findPlayersForRerun(contractId, day, hours, isUltraOnly) {
	...
	for (const tabName of tabs) {
		try {
			const player = await findPlayerForRerun(tabName, contractId, day, hours, isUltraOnly);
			if (player) {
				players.push(player);
			}
		} catch (error) {
		...
		for (let i = 1; i <= 7; i++) {
			...
			for (const rowIndexStr in ROW_TO_EST) {
				...
				if (
					...
					availableHours.push(ROW_TO_EST[rowIndex]);
				}
			}
			schedule.push(convertEstToUtc(availableHours));
		}
		return schedule;
	} catch (error) {
		throw error;
...
export async function updateScheduleInSheet(sheetTab, dayOfWeek, utcHours) {
	try {
		...
		const columnIndex = dayOfWeek + 1; // B=1, C=2, ... H=7
